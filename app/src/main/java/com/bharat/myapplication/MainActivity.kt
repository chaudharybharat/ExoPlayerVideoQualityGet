...
class MainActivity : AppCompatActivity(),View.OnClickListener,ManifestFetcher.ManifestCallback<HlsPlaylist>,SurfaceHolder.Callback,ExoPlayer.Listener {
              ...
              val popup = PopupMenu(this, v)
               popup.setOnMenuItemClickListener(object :PopupMenu.OnMenuItemClickListener{
                  override fun onMenuItemClick(item: MenuItem?): Boolean {
                      Log.e("test","select truck"+(item!!.groupId-1))
                      player!!.setSelectedTrack(1,(item!!.groupId-1))
    ...
    override fun onSingleManifest(manifest: HlsPlaylist?) {
       ...
       var chunkSource=HlsChunkSource(true,dataSource,manifest,DefaultHlsTrackSelector.newDefaultInstance(this),bandWithMetor,timestampAdjusterProvider,HlsChunkSource.ADAPTIVE_MODE_SPLICE)
      var sampleSource= HlsSampleSource(chunkSource,loadControl,MAIN_BUFFER_SUGMENT*BUFFER_SUGMENT_SIZE)
       videorender=MediaCodecVideoTrackRenderer(this,sampleSource, MediaCodecSelector.DEFAULT,MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT)
        audiorender= MediaCodecAudioTrackRenderer(sampleSource, MediaCodecSelector.DEFAULT)
      player!!.prepare(videorender,audiorender)
      player!!.sendMessage(videorender,MediaCodecVideoTrackRenderer.MSG_SET_SURFACE,surfaceView.holder.surface)
    ...
    override fun surfaceCreated(holder: SurfaceHolder?) {
        if(videorender!=null){
            player!!.blockingSendMessage(videorender,MediaCodecVideoTrackRenderer.MSG_SET_SURFACE,surfaceView)
        }
